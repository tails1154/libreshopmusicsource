...
static lwp_t background_thread = LWP_THREAD_NULL;
...
static void* loop_sound(void* arg) {
    while (1) {
        if (loading) {
            ...
            if (MP3Player_IsPlaying()) {
                MP3Player_Stop();  // Stop if loading is false
            }
        }
        usleep(100 * 1000); // Sleep for 100ms to reduce CPU usage
    }
    return NULL;
}
...
int main(int argc, char **argv) {
	...
	printf("\x1b[1;0H");
     // Start the loading sound thread
    if (LWP_CreateThread(
        &background_thread,
        loop_sound,  // Function to run
        NULL,           // No arguments
        NULL,           // Default stack
